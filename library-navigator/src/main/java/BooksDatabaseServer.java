...
public class BooksDatabaseServer {
    ...
    public BooksDatabaseServer(){
        ...
        try {
            ...
            this.serverSocket = new ServerSocket(this.thePort, 5, InetAddress.getByName(theIPAddress));
    ...
    public void executeServiceLoop()
    {
        ...
        try {
            ...
            while (true) {
                //TO BE COMPLETED
                Socket tmp1 = this.serverSocket.accept();
                BooksDatabaseService b1 = new BooksDatabaseService(tmp1);
                Thread bThread = new Thread(b1);
                bThread.start();
                System.out.println("Thread created"+bThread);
            }
        } catch (Exception e){
            //The creation of the server socket can cause several exceptions;
            //See https://docs.oracle.com/javase/7/docs/api/java/net/ServerSocket.html
